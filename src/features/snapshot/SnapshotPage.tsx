            ...
            <div key={i} className="recent-activity-item">
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                  <span className="recent-activity-label">
                    {a.label}
                    {a.category ? (
                      <span style={{ fontSize: '0.72rem', color: 'var(--muted)', fontWeight: 400 }}> · {getCategoryName(cfg, a.category)}{a.subcategory ? ` · ${a.subcategory}` : ''}</span>
                    ) : null}
                  </span>
                  {a.amount != null ? <span style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--ui-primary-text, var(--text))', flexShrink: 0 }}>{formatCents(a.amount)}</span> : null}
                </div>
                <div className="recent-activity-type">
                  {a.descriptor ?? a.type} · {timeStr}
                </div>
              </div>
            </div>
          );
...
export function SnapshotPage({
  ...
  // Recompute investing state whenever main data changes, since store actions may
  // call saveInvesting() as a side effect (e.g. posting HYSA pending items, HYSA purchases).
